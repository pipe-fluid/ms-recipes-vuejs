	...
	<div class="text-center">
		<v-dialog v-model="dialog" width="500">
			...
			<v-card>
				...
				<!-- Chefs -->
				<v-card-text :chefNames="chefNames" v-if="recipes.chef">
					Chef: {{ chefNames[recipes.chef.sys.id] }}</v-card-text
				>
				<v-card-text v-else> Chef: N/A</v-card-text>
				<v-divider></v-divider>
...
export default {
	name: "Details",
	props: ["recipes", "photoUrl", "chefNames"],
	data() {
		return {
			dialog: false,
		};
